## Всем привет! 👋
Меня зовут Иван Буренин, мне 21 год, я являюсь Front-end разработчиком. Качество выполненного  продукта является приоритетом для меня. При работе гарантирую вежливое общение и обратную связь по верстке.

## Чем могу помочь
<ul>
    <li>
        🖥️ Сверстаю сайт любой сложности, от лендинга до интернет-магазина по макету;
    </li>
    <li>
        📖 Создам верстку специально под Wordpress;
    </li>
    <li>
        📦 Перенесу готовую верстку на Wordpress;
    </li>
    <li>
        🔧 Исправлю верстку или стили;
    </li>
</ul>

## Навыки
<div>
    <div>
        <img src='/skills-images/html-skill.png'>
        <img src='/skills-images/css-skill.png'>
        <img src='/skills-images/js-skill.png'>
    </div>
    <div>
        <img src='/skills-images/react-skill.png'>
        <img src='/skills-images/next-skill.png'>
        <img src='/skills-images/redux-skill.png'>
    </div>
    <div>
        <img src='/skills-images/figma-skill.png'>
        <img src='/skills-images/ps-skill.png'>
    </div>
    <div>
        <img src='/skills-images/wp-skill.png'>
    </div>
</div>

## Связаться со мной
Если вы хотите сделать заказ или обсудить возможное сотрудничество, не стесняйтесь связаться со мной. С радостью обсудим детали и начнем работу над вашим проектом. Приглашаю к сотрудничеству:

<div>
    <a href='https://vk.com/ivanburenin77'>
        <img src='/links-images/vk.png' style='border-radius: 10px'>
    </a>
    <a href='mailto:stylespeedquality@mail.ru'>
        <img src='/links-images/mail.png' style='border-radius: 10px'>
    </a>
    <img src='/links-images/tg.png'>
</div>
